...
const SectionC = ({ data, onChange, onBack, onNext }) => {
  ...
  const isStepValid = () => {
    ...
    if (data.hasMaintenanceClaims === 'yes') {
      return data.maintenancePersonDetails && data.maintenancePersonDetails.trim().length > 0;
    }
    return true;
  };
      ...
      <div className="input-group mb-25">
        ...
        <div className="section-c-question-container">
          <span className="question-text">
            Haben Sie Angehörige, die Ihnen gegenüber gesetzlich zur Leistung von Unterhalt verpflichtet sind (auch wenn tatsächlich keine Leistungen erfolgen)?
          </span>
          <span className="section-c-subtext">
            z. B. Mutter, Vater, Ehegatte/Ehegattin, eingetragene(r) Lebenspartner/Lebenspartnerin
          </span>
        </div>
          ...
          >
            <div className="warning-box-c">
              ...
              <div className="warning-text">
                ...
                Bitte geben Sie auf einem weiteren Exemplar dieses Formulars seine persönlichen und wirtschaftlichen Verhältnisse an, sofern diese nicht bereits vollständig aus den folgenden Abschnitten ersichtlich sind.
              </div>
            </div>

            <label className="input-label-c">
              Name des Unterhaltsverpflichteten <span style={{color: 'red'}}>*</span>
            </label>
            ...
            <textarea 
              ...
              onChange={(e) => onChange('maintenancePersonDetails', e.target.value)}
              rows={2}
              style={!data.maintenancePersonDetails ? { borderColor: '#e2e8f0' } : {}}
            />
          </motion.div>
        )}
      </div>
