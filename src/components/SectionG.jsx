...
const SectionG = ({ data, onChange, onBack, onNext }) => {
  ...
  // ИСПРАВЛЕНО: Обновляем только конкретное поле (key), а не весь объект
  const handleToggle = (key, status) => {
    ...
    if (status === 'no') {
      ...
      // Сохраняем введенные данные, если пользователь передумал и вернул "Да"
      newItem = { ...currentItem, has: 'yes' };
    }
    
    // Передаем (имя_поля, значение)
    onChange(key, newItem);
  };

  // ИСПРАВЛЕНО: Точечное обновление полей ввода
    ...
    <div className="section-card">
      ...
      <div className="info-box-alert">
        ...
        <div className="info-text-content">
          <h4>Ihr Vermögen</h4>
          <p>
            Verfügen Sie oder Ihr Ehegatte/Partner allein oder gemeinsam über folgende Vermögenswerte?
          </p>
        </div>
      </div>
      ...
      <div className="asset-list">
        {categories.map((cat) => {
          const itemData = data[cat.key] || { has: null, description: '', value: '' };
            ...
            >
              ...
              {itemData.has === 'yes' && (
                <div className="form-grid-assets">
                  <div>
                    <label className="input-label-small">Beschreibung / Details</label>
                    <textarea
                  ...
                  <div>
                    <label className="input-label-small">Wert in €</label>
                    <input
                      type="text"
      ...
      <div className="navigation-footer">
        ...
        <button className="btn-primary-action" onClick={onNext}>Weiter zu Abschnitt H</button>
      </div>
    </div>
  );
};
