...
const SectionE = ({ data, onChange, onBack, onNext, setStep }) => {
	...
	const incomeCategories = [
		...
		{
			key: 'sonstige',
			label: 'Andere Einnahmen (z.B. Urlaubsgeld)',
			isOther: true,
		},
	]
	...
	const isIncomeStepValid = personType => {
		return incomeCategories.every(cat => {
			...
			if (item?.has === 'yes') {
				if (cat.key === 'sonstige' && item.items) {
					return item.items.every(i => i.brutto && i.brutto.trim() !== '')
				}
				return item.brutto && item.brutto.trim() !== ''
			}
			return true
		})
	}
		...
		return (
			<div className='income-list'>
				...
				{incomeCategories.map(cat => (
					<div
						key={cat.key}
						className={`income-card ${data[personType]?.[cat.key]?.has === 'yes' ? 'active' : ''}`}
					>
						<div className='income-header'>
							...
							<div className='income-controls'>
								<button
									...
									onClick={() =>
										updateIncome(personType, cat.key, 'has', 'yes')
									}
								>
									Ja
								</button>
							...
							>
								{cat.isOther ? (
									<div className='andere-einnahmen-box'>
										...
										>
											<strong>Hinweis:</strong> Belege (z. B. Lohnbescheinigung,
											Steuerbescheid, Bewilligungsbescheid mit Berechnungsbogen)
											müssen in Kopie beigefügt werden.
										</div>

										{(
											data[personType].sonstige.items || [
												{ details: '', brutto: '' },
											]
										).map((item, index) => (
											<div
												key={index}
												className='other-income-item'
												style={{
													marginBottom:
														index === 0 &&
														data[personType].sonstige.items?.length > 1
															? '20px'
															: '0',
													paddingBottom:
														index === 0 &&
											...
											>
												<div className='mb-15'>
													<label className='input-label-small'>
														{index === 0
															? 'Art & Zeitraum (Eintrag 1)'
															: 'Art & Zeitraum (Eintrag 2)'}
													</label>
													<textarea
														rows='1'
													...
													<p className='income-hint'>
														Geben Sie bitte Art, Bezugszeitraum u Höhe an.
													</p>
												</div>
												<div>
													<label className='input-label-small'>
												...
												{index === 1 && (
													<button
														onClick={() =>
															removeOtherIncomeItem(personType, index)
														}
														style={{
															marginTop: '10px',
															color: '#ef4444',
										...
										{(!data[personType].sonstige.items ||
											data[personType].sonstige.items.length < 2) && (
											<button
				...
				{allNo && (
					...
					>
						<div className='mb-15'>
							...
							>
								5. Sie haben alle Fragen zu Ihrem Einkommen mit „Nein“
								beantwortet:
							</label>
							<p
								style={{
							...
							>
								Sie geben an, keine Einkünfte zu haben. Bitte erläutern Sie auf
								einem separaten Blatt, wie Sie Ihren Lebensunterhalt bestreiten.
							</p>
							<p
								style={{
									fontSize: '0.9rem',
							...
							>
								Hinweis: Dieses Dokument muss der Erklärung separat beigefügt
								werden.
							</p>
						</div>
					</motion.div>
				...
				<div className='navigation-footer'>
					...
					<button
						...
						title={
							!isIncomeStepValid(personType)
								? 'Bitte füllen Sie alle Beträge für ausgewählte Einnahmen aus'
								: ''
						}
					>
						Weiter
		...
		<div className='section-card'>
			...
			<AnimatePresence mode='wait'>
					...
					>
						<div className='info-box-alert'>
							...
							<div className='info-text-content'>
								Wenn Sie <b>Ja</b> wählen, werden die Abschnitte <b>E bis J</b>{' '}
								automatisch übersprungen.
							</div>
						</div>
						<div className='substep-instruction'>
							Beziehen Sie laufende Leistungen zum Lebensunterhalt nach dem SGB
							XII (Sozialhilfe)? <span style={{ color: 'red' }}>*</span>
						</div>
						<div className='button-row-sgb'>
							<button
								className={`sgb-btn-large ${data.receivesSocialAssistanceSGBXII === 'yes' ? 'active-ja' : ''}`}
								onClick={() =>
							...
							<button
								className={`sgb-btn-small ${data.receivesSocialAssistanceSGBXII === 'no' ? 'active-nein' : ''}`}
						...
						<div className='navigation-footer'>
							...
							<button
								...
								style={{
									background:
										data.receivesSocialAssistanceSGBXII !== null
											? 'var(--accent-color)'
											: '#ccc',
								}}
							>
								{data.receivesSocialAssistanceSGBXII === 'yes'
									? 'Direkt zu K'
									: 'Weiter'}
							</button>
						</div>
				...
				{subStep === 1 && (
					...
					>
						<h3>Ihre Bruttoeinnahmen </h3>
						{renderIncomeList('self')}
					</motion.div>
				)}
				{subStep === 2 && (
					<motion.div
					...
					>
						...
						<div className='button-row-sgb'>
							<button
								className={`sgb-btn-large ${data.hasPartner === 'yes' ? 'active-ja' : ''}`}
								onClick={() => onChange('hasPartner', 'yes')}
							>
								Ja
							</button>
							<button
								className={`sgb-btn-small ${data.hasPartner === 'no' ? 'active-nein' : ''}`}
						...
						<div className='navigation-footer'>
							...
							<button
								...
								style={{
									background:
										data.hasPartner !== null ? 'var(--accent-color)' : '#ccc',
								}}
							>
								   {data.hasPartner === 'yes' 
                                    ? 'Weiter zu Einnahmen des Partners' 
                                    : 'Weiter zu Abschnitt F'}
							</button>
						</div>
					</motion.div>
				)}
				{subStep === 3 && (
					<motion.div
						...
						animate={{ x: 0, opacity: 1 }}
					>
						<h3>Bruttoeinnahmen des Partners</h3>
						{renderIncomeList('partner')}
					</motion.div>
				)}
			</AnimatePresence>
		</div>
